...
### 📅 과거 날짜 조회 — 실측을 직접 볼 수 있습니다
16일 스트립은 미래(예보)만 다룹니다. 스트립 위 **"날짜로 이동"** 입력에 과거 날짜를
넣으면, 그날의 실측 파고·바람·조위·날씨를 그대로 보여줍니다.

| 구간 | 파도(marine) | 기상(바람·기온·날씨) |
|---|---|---|
| 최근 ~90일 | marine 모델 실측 | 예보 API 관측 블렌딩 (강수 **확률**) |
| 그 이전 (최대 약 5년) | marine 모델 실측 | ERA5 재분석 (강수 **실측 mm**) |
| 5년보다 오래 전 | 없음 — 정직하게 "파도 데이터가 없습니다" | — |

**하드코딩된 컷오프 날짜가 없습니다.** Open-Meteo 의 실측 창이 넓어지거나 좁아져도,
응답이 실제로 비어 있는지로 판정하므로 코드를 고칠 필요가 없습니다.
확률(forecast API)과 실측 강수량(ERA5, mm)은 **다른 값**이라 행 자체가 바뀌어
표시됩니다 — 실측을 확률인 척 보여주지 않습니다.
